...
// Reset onboarding (but keep user doc)
async function resetOnboarding(senderId) {
  const collection = await connectToProfiles();
  if (!collection) return false;
  ...
  try {
    await collection.updateOne(
      ...
      { 
        $set: { 
          ...
          'foodProfile.dietary': [],
          'foodProfile.budget': null,
          'foodProfile.borough': null,
          'foodProfile.craving': null,
          'foodProfile.favSpots': [],
          'context.lastCategory': null,
          'context.lastFilters': null,
          'context.shownDedupeKeys': [],
          'context.shownNames': [],
    ...
    console.error('Error resetting onboarding:', err.message);
    return false;
  }
}
